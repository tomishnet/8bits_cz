...
<?php
    if ($showPaging && $numpages > 1) {
      ...
      if (function_exists('return_pagify_navigation')) {
        ...
        $pagingHtml = '<div class="row"></div><div class="starsi pagination">';
        ...
        if (@$i18n['PREV_TEXT'] && $pageNum > 0) {
          $pagingHtml .= '<span class="previous"><a href="'.str_replace('%PAGE%',$pageNum,$link).'" title="'.htmlspecialchars(@$i18n['PREV_TITLE']).'"><span class="fa fa-arrow-circle-left"></span>&nbsp;'.htmlspecialchars(@$i18n['PREV_TEXT']).'&nbsp;|&nbsp;</a></span>';
        }
        for ($i=0; $i<$numpages; $i++) {
          if ($i == $pageNum) {
            //$pagingHtml .= ' <span class="current">'.($i+1).'</span>';
            $pagingHtml .= ' <span><a  href="'.str_replace('%PAGE%',$i+1,$link).'"  class="active">'.($i+1).'</a></span>&nbsp;|&nbsp;';
          } else {
            $pagingHtml .= ' <span><a href="'.str_replace('%PAGE%',$i+1,$link).'">'.($i+1).'</a></span>&nbsp;|&nbsp;';
          }
        }
        if (@$i18n['NEXT_TEXT'] && $pageNum < $numpages-1) {
          $pagingHtml .= ' <span class="next"><a href="'.str_replace('%PAGE%',$pageNum+2,$link).'" title="'.htmlspecialchars(@$i18n['NEXT_TITLE']).'">'.htmlspecialchars(@$i18n['NEXT_TEXT']).'&nbsp;<span class="fa fa-arrow-circle-right"></span></a></span>';
        }
