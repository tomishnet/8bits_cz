...
function clientfiles_display($contents)
{
    ...
    if ($location !== FALSE)
    {
        ...
        if ($goodlogin === FALSE)
        {
            $clientfiles_content .= 'Podívejte se, v jakém stavu je vaše zakázka.<br /><br />';

            // login form
            $clientfiles_content .= '<form name="login" action="#clientlogin" method="post">';
            $clientfiles_content .= '<label for="client">Login klienta:</label><br><input type="text" style="width: 150px" name="client" value="">';
            $clientfiles_content .= '&nbsp;&nbsp;</br><label for="password">Heslo:</label><br><input type="password" style="width: 150px" name="pass" value="">';
            $clientfiles_content .= '<br><input type="submit" name="submitlogin" value="Přihlásit" />';
        ...
        else // logged in display stuff
        {
            ...
            $clientfiles_content .= '<div class="datagrid"><table id="cf_table">';
            $clientfiles_content .= '<caption>Klient: ' . $client . '</caption>';
            $clientfiles_content .= '<thead><tr><th>Soubor</th><th>Datum</th></tr></head>';
                ...
                $rowclass="";
            {
                foreach ($filearray as $clientfile)
                {
                    $clientfiles_content .= '<tr' . $rowclass . '><td><a href="' . $SITEURL . 'plugins/clientfiles/dlfile.php?client=' . urlencode($client)
                        . '&getfile=' . urlencode($clientfile[0]) . '" title="Stáhnout soubor">' . $clientfile[0]
            ...
            if ($fullcount==1)
            {
                $clientfiles_content .= "$fullcount soubor";
            }
            else if ($fullcount==0)
            {
                $clientfiles_content .= "žádný soubor";
            }
            else
            {
                $clientfiles_content .= "$fullcount souborů";
            }
            ...
            if (floor(($fullcount-1)/$perpage) > 0)
            {
                ...
                $thispage = find_url($url, $parent);
                if ((string)$PRETTYURLS==="1")
                {
                    $thispage .= "?";
                }
                else
            ...
            $clientfiles_content .= '</tbody></table></div><br />';

            // logout form
            $clientfiles_content .= '<form name="logout" action="#clientlogout" method="post">';
            $clientfiles_content .= '<input type="submit" name="submitlogout" value="Odhlášení klienta" />';
            $clientfiles_content .= '</form></br>';
        }
