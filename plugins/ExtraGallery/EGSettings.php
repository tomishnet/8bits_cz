...
class EGSettings {
	...
	*/
    ...
    public static function save($instanceNum){
        ...
        $xml->addChild('required-width')->addCData($data['required-width']);     

		$xml->addChild('required-height-comparator')->addCData($data['required-height-comparator']);
        $xml->addChild('required-height')->addCData($data['required-height']);
    ...
    private static function _validate($data){
		...
		if ($data['required-width'] !== '' && (int)$data['required-width'] <= 0)
			return false;  	

		if ($data['required-height'] !== '' && (int)$data['required-height'] <= 0)
			return false;      
