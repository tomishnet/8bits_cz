<?php
i18n_gallery_register('swipebox', 'swipebox',
    '<strong>swipebox</strong> display simple list of thumbs, all js code is in template swipebox<br/>'.
    'License: Creative Commons Attribution 2.5<br/>'.
    '<a target="_blank" href="http://www.tomish.net">http://www.tomish.net</a>',
    'i18n_gallery_swipebox_edit', 'i18n_gallery_swipebox_header', 'i18n_gallery_swipebox_content');

function i18n_gallery_swipebox_edit($gallery) {
    ?>
    <p>
        <label for="swipebox-width"><?php i18n('i18n_gallery/MAX_DIMENSIONS'); ?></label>
        <input type="text" class="text" id="swipebox-width" name="swipebox-width" value="<?php echo @$gallery['width']; ?>" style="width:5em"/>
        x
        <input type="text" class="text" id="swipebox-height" name="swipebox-height" value="<?php echo @$gallery['height']; ?>" style="width:5em"/>
        <input type="hidden" name="swipebox-crop" value="1"/>
    </p>
    <p>
        <label for="swipebox-textpos"><?php i18n('i18n_gallery/TEXT_POSITION'); ?></label>
        <select class="text" name="swipebox-textpos">
    ...
    <p>
        <label for="swipebox-interval"><?php i18n('i18n_gallery/INTERVAL'); ?></label>
        <input type="text" class="text" id="swipebox-interval" name="swipebox-interval" value="<?php echo @$gallery['interval']; ?>" style="width:5em"/>
    </p>
<?php
}

function i18n_gallery_swipebox_header($gallery) {
    $id = 'swipebox-'.i18n_gallery_id($gallery);
    ...
    if (i18n_gallery_check($gallery,'js') && i18n_gallery_needs_include('swipebox.js')) {
        // packed version of js does not work!
        ?>
        <script type="text/javascript" src="<?php echo i18n_gallery_site_link(); ?>plugins/i18n_gallery/js/swipebox.js"></script>
    <?php
    }
...
function i18n_gallery_swipebox_content($gallery) {
    $id = 'swipebox-'.i18n_gallery_id($gallery);
    ?>
...
//    exit;
    ...
    foreach ($gallery['items'] as $k_item => $item) {
        ...
        ?>
        <div class="col-xs-12 col-md-3" data-thumb="<?php i18n_gallery_thumb_link($gallery, $item); ?>">
            <div class="panel panel-default animate">
                <div class="panel-image hide-panel-body">
                    <a href="<?php i18n_gallery_image_link($gallery, $item); ?>" class="swipebox_<?php echo $gallery["name"]; ?>" data-thumb="<?php i18n_gallery_thumb_link($gallery, $item); ?>" data-url="<?php i18n_gallery_image_link($gallery, $item); ?>" title="<?php echo htmlspecialchars(@$item['_title']); ?> <?php echo htmlspecialchars(@$item['_description']); ?>">
                        <img src="<?php i18n_gallery_thumb_link($gallery, $item); ?>" data-thumb="<?php i18n_gallery_thumb_link($gallery, $item); ?>" data-url="<?php i18n_gallery_image_link($gallery, $item); ?>" alt="<?php echo htmlspecialchars(@$item['_title']); ?> <?php echo htmlspecialchars(@$item['_description']); ?>" class="img-responsive">
                    </a>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $(".swipebox_<?php echo $gallery["name"];?>").swipebox();
        });
    </script>
    <?php
