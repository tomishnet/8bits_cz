...
// die();
  ...
  if (@$defs && count($defs) > 0) {
    ...
    echo '<div class="spe_special-container" style="clear:both">';
	$i = 0;
    foreach ($defs as $def) {
		$i++;
		...
		echo '<div class="spe_cell" '. ($hasWidth ? 'style="width:'.$w.'"' : '') .'>';
	  
  		switch ($type){
			case 'text':
			case 'textfull':{ 
				echo '<b>'.$label.':</b><br />';
				echo '<input class="text" type="text" id="post-sp-'.$key.'" name="post-sp-'.$key.'" value="'.$value.'" />';  
				break; 
			}
  			case 'dropdown':{
				echo '<b>'.$label.':</b><br />';
				echo '<select id="post-sp-'.$key.'" name="post-sp-'.$key.'" class="text" >';
